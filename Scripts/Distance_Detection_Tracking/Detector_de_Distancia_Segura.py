...
while True:
    t1 = cv2.getTickCount()  # Start time recording
    frame1 = videostream.read()

    if frame1 is None:
        print(
            "Failed to capture frame from camera. Check camera connection and try again."
        )
        continue
    ...
    person_detected_under_3m = False
    ...
    for i in range(len(scores)):
        if scores[i] > min_conf_threshold and scores[i] <= 1.0:
            ...
            if object_name == "person" and distance < 3:
                person_detected_under_3m = True
                break  # No need to continue looping if we found a person within 3m
    ...
    if person_detected_under_3m:
        print("Person detected within 3 meters. Turning off the car.")
